...
for datei in dateien:
    # kc-Wert extrahieren
    if dim2 == True:
        match = re.search(r'kcoupl_xy=(\d+(?:\.\d+)?)', datei) # xy
    elif dim1 == True:
        match = re.search(r'kcoupl=(\d+(?:\.\d+)?)', datei)  # x
    if match:
...
for i, (kc_wert, frequenz, transmission) in enumerate(plots):
    ...
    if dim2 == True:
        '''if i==1:
            kc_wert = 100.5 * 9
            label = r"$k_\mathrm{C}^{\text{x}}$ = " + f"{kc_wert / 9}" + r"$\,\frac{\text{meV}^2}{Å^2}$" if kc_wert is not None else "kc unbekannt"
        else:
            kc_wert = 100 * 9  
            label = r"$k_\mathrm{C}^{\text{x}}$ = " + f"{int(kc_wert / 9)}" + r"$\,\frac{\text{meV}^2}{Å^2}$" if kc_wert is not None else "kc unbekannt"'''
...
if logscale:
    ax.set_ylabel(r'$\text{log}\left(\tau_{\mathrm{ph}}\right)$', fontsize=28, fontproperties=prop)
    ax.set_yscale('log')  # Logarithmische Y-Achse für bessere Sichtbarkeit
    #ax.set_xscale('log')
else:
    ax.set_ylabel(r'$\tau_{\mathrm{ph}}$', fontsize=28, fontproperties=prop)
...
ax.grid(zorder=1, alpha=0.3)  # Grid ganz hinten
...
if logscale:
    ...
    plt.savefig(dest_fld + "/transmission_comb.svg", bbox_inches='tight')
